...
export default function CreateTrip({ loaderData }: Route.ComponentProps) {
  ...
  const comboRefs = useRef<Record<string, any>>({});
  ...
  return (
    <>
      <main className="flex flex-col gap-10 pb-20 wrapper">
        ...
        <section className="mt-2.5 w-full max-w-5xl">
          <form onSubmit={handleSubmit} className="trip-form ">
            <div>
              ...
              <ComboBoxComponent
                id="country"
                ref={(el: any) => {
                  if (el) comboRefs.current["country"] = el;
                }}
                focus={() => {
                  comboRefs.current["country"]?.showPopup(); // 👈 Show on focus
                }}
            ...
            {selectItems.map((key) => (
              <div key={key}>
                ...
                <ComboBoxComponent
                  id={key}
                  ref={(el: any) => {
                    if (el) comboRefs.current[key] = el;
                  }}
                  focus={() => {
                    comboRefs.current[key]?.showPopup(); // 👈 Show on focus
                  }}
